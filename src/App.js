import { useEffect, useState } from "react";
// import Payment from "./components/Payment";
import { addPayment, getAllPayment, updatePayment, deletePayment } from "./utils/HandleApi";
import './components/FormValidate'



function App() {


  const [payment, setPayment] = useState([])
  const [email, setEmail] = useState("")
  const [card_holder_name, setCard_holder_name] = useState("")
  const [card_number, setCard_number] = useState("")
  const [end_date, setEnd_date] = useState("")
  const [cvv_number, setCvv_number] = useState("")
  const [isUpdating, setIsupdating] = useState(false)
  const [paymentId, setPaymentId] = useState("")


  

  useEffect(() => {
    getAllPayment(setPayment)
  }, [])


  const updateMode =(_id, email, card_holder_name, card_number, end_date, cvv_number) => {
    setIsupdating(true)
    setEmail(email)
    setCard_holder_name(card_holder_name)
    setCard_number(card_number)
    setEnd_date(end_date)
    setCvv_number(cvv_number)
    setPaymentId(_id)
  }



  return (
    <div className="home">
      <div className="container">
        <h1>Payment Management System</h1>






        <div ng-app="app" class="container">
        <form ng-app="app"  ng-controller="PaymentFormCtrl" class="payment-form" name="paymentForm" >
          <div class="notification">
            {/* <img src="data:image/png;base64,"  class="notification__icon"/> */}
            {/* <span class="notification__text">
            We do not store your payment data, but immediately transfer it to the payment system via an encrypted channel
            </span> */}
          </div>
          <div class="card-type clearfix">
            <div class="card-type__label">
              Enter credit card details
            </div>
            <div class="card-type__icons" >
              {/* <img src="data:image/png;base64," class="card-type__icon card-type__icon--disabled" ng-class="{'card-type__icon--disabled': !isVisa()}" /> */}
              {/* <img src="data:image/png;base64," class="card-type__icon card-type__icon--disabled" ng-class="{'card-type__icon--disabled': !isMasterCard()}"/> */}
            </div>
          </div>





          
          <input type="email" class="card-input card-input--full" placeholder="Email"  required value={email} onChange={(e) => setEmail(e.target.value)}/>



        <input type="text" class="card-input card-input--full" data-mask="0000 0000 0000 0000" placeholder="Name of the holder (as on the card)" ng-model="card.holder" card-holder required value={card_holder_name} onChange={(e) => setCard_holder_name(e.target.value)}/>

            
         
          <input type="text" class="card-input card-input--full" pattern="[0-9]*" inputmode="numeric" placeholder="Card number" ng-model="card.number" required card-number maxLength={16}
          value={card_number} onChange={(e) => setCard_number(e.target.value)}/>

            

          



          <div class="card-info clearfix">
            <div class="card-info__date">
              {/* <div class="card-info__text">
                Card Expire date
              </div> */}
              <input type="text" class="card-input card-input--date" data-mask="00 / 00" placeholder="MM / YY" ng-model="card.date" card-date required maxLength={5}
              value={end_date} onChange={(e) => setEnd_date(e.target.value)} />
            </div>
            <div class="card-info__cvv">
              {/* <div class="card-info__text">
                Card PIN Number
              </div> */}
              <input type="text" class="card-input card-input--cvv" placeholder="CVV" ng-model="card.cvv" card-cvv required maxLength={3}
              value={cvv_number} onChange={(e) => setCvv_number(e.target.value)}/>
            </div>
          </div>

          <button type="submit" class="btn btn-primary" onClick={ isUpdating ? 
          () => updatePayment(paymentId, email, setEmail, card_holder_name, setCard_holder_name, card_number, setCard_number, 
            end_date, setEnd_date, cvv_number, setCvv_number, setPayment, setIsupdating ) 
          : () => 
          addPayment(email, setEmail, card_holder_name, setCard_holder_name, card_number, setCard_number, 
          end_date, setEnd_date, cvv_number, setCvv_number, setPayment)} >{isUpdating ? "Update" : "Add"}</button>

          <div class="payment-form__agreement">
          </div>
        </form>
        
        {/* <script src="https://raw.githubusercontent.com/candreoliveira/ngMask/master/dist/ngMask.min.js"></script> */}

      </div>














































        {/* <div className="container">

           <form className="mt-4" 
            // onSubmit={updateuser}
           >
           
               <div className="row">
                   <div class="mb-3 col-lg-6 col-md-6 col-12">
                       <label for="exampleInputEmail1" class="form-label">Email</label>
                       <input type="text" required value={email} onChange={(e) => setEmail(e.target.value)}  class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" />
                   </div>

                   <div class="mb-3 col-lg-6 col-md-6 col-12">
                       <label for="exampleInputEmail1" class="form-label">Card Holder Name</label>
                       <input type="text" required value={card_holder_name} onChange={(e) => setCard_holder_name(e.target.value)}  class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" />
                   </div>

                   <div class="mb-3 col-lg-6 col-md-6 col-12">
                       <label for="exampleInputEmail1" class="form-label">Card Number</label>
                       <input type="text" required value={card_number} onChange={(e) => setCard_number(e.target.value)}  class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" />
                   </div>

                   <div class="mb-3 col-lg-6 col-md-6 col-12">
                       <label for="exampleInputEmail1" class="form-label">Card Expire Date</label>
                       <input type="text" required value={end_date} onChange={(e) => setEnd_date(e.target.value)}  class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" />
                   </div>

                   <div class="mb-3 col-lg-6 col-md-6 col-12">
                       <label for="exampleInputEmail1" class="form-label">CVV Number</label>
                       <input type="text" required value={cvv_number} onChange={(e) => setCvv_number(e.target.value)}  class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" />
                   </div>

                   <button type="submit" class="btn btn-primary" onClick={ isUpdating ? 
          () => updatePayment(paymentId, email, setEmail, card_holder_name, setCard_holder_name, card_number, setCard_number, 
            end_date, setEnd_date, cvv_number, setCvv_number, setPayment, setIsupdating ) 
          : () => 
          addPayment(email, setEmail, card_holder_name, setCard_holder_name, card_number, setCard_number, 
          end_date, setEnd_date, cvv_number, setCvv_number, setPayment)} >{isUpdating ? "Update" : "Add"}</button>
               
               </div>
           </form>
       </div> */}

</div>

      <div className="header_fixed">
<table>
    {/* <thead>
        <tr>
            <th>Email</th>
            <th>Card holder name</th>
            <th>Card number</th>
            <th>Expire date</th>
            <th>CVV number</th>
            <th>Edit</th>
            <th>Delete</th>         
        </tr>
    </thead> */}
    <tbody>



      {/* original code segment */}
{/* 
        <div className="list">
          
          {payment.map((item) => <Payment key={item._id}
          email={item.email}
          card_holder_name={item.card_holder_name}
          card_number={item.card_number}
          end_date={item.end_date}
          cvv_number={item.cvv_number}
          updateMode={() => updateMode(item._id, item.email, item.card_holder_name, item.card_number, item.end_date, item.cvv_number)}
          deletePayment={() => deletePayment(item._id, setPayment)}
          />)}
         

        </div> */}
 {payment.map(item => (
            <tr key={item._id}>
            <td>{item.email}</td>
            <td>{item.card_holder_name}</td>
            <td>{item.card_number}</td>
            <td>{item.end_date}</td>
            <td>{item.cvv_number}</td>
            <td><button onClick={() => updateMode(item._id, item.email, item.card_holder_name, item.card_number, item.end_date, item.cvv_number)}>Edit</button></td>
            <td><button onClick={() => deletePayment(item._id, setPayment)}>Delete</button></td> 
            </tr>
          ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}

export default App;
